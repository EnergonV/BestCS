...
//
// Copyright © BestCS.NET, 2009-2013
// dinruspro@mail.ru
//
...
namespace  BestCS.Video.DirectShow
{
    ...
    using  BestCS.Video;
    using  BestCS.Video.DirectShow.Internals;
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        public void DisplayPropertyPage( IntPtr parentWindow )
        {
            ...
            lock ( sync )
            {
                ...
                {
                    throw new NotSupportedException( "Видеоисточник не поддерживает страницу видеоконфигурации." );
                }
        ...
        public void DisplayCrossbarPropertyPage( IntPtr parentWindow )
        {
            lock ( sync )
            {
                ...
                {
                    throw new ApplicationException( "Требуется запуск видеосточника для отображения страницы свойств crossbar." );
                }

                if ( !isCrossbarAvailable.Value )
                {
                    throw new NotSupportedException( "Конфигурация Crossbar не поддерживается текущим видеоисточником." );
                }
        ...
        public bool GetCameraProperty( CameraControlProperty property, out int value, out CameraControlFlags controlFlags )
        {
            ...
            if ( ( deviceMoniker == null ) || ( string.IsNullOrEmpty( deviceMoniker ) ) )
            {
                throw new ArgumentException( "Видеисточник не указан." );
            }
                ...
                if ( streamConfig != null )
                {
                    if ( capabilities == null )
                    {
                        try
                        {
                            ...
                            capabilities =  BestCS.Video.DirectShow.VideoCapabilities.FromStreamConfig( streamConfig );
                        }
                        catch
                        {
                        }
