...
namespace  BestCS.Imaging
{
    ...
    public abstract class BlobCounterBase
    {
        ...
        /// useful in conjunction with different routines from <see cref=" BestCS.Math.Geometry"/>,
        ...
        private unsafe void CollectObjectsInfo( UnmanagedImage image )
        {
            ...
            for ( int j = 1; j <= objectsCount; j++ )
            {
                ...
                blob.CenterOfGravity = new  BestCS.Point( (float) xc[j] / blobArea, (float) yc[j] / blobArea );
