        ...
        private static WavechartBox show(String title, bool hold, params Tuple<Signal, int>[] series)
        {
            ...
            formThread = new Thread(() =>
            {
                ...
                for (int i = 0; i < series.Length; i++)
                {
                    ...
                    {
                        double[] spectrum = BestCS.Audio.Tools.GetPowerSpectrum(complex.GetChannel(channel));
                        double[] frequencies = BestCS.Audio.Tools.GetFrequencyVector(signal.Length, signal.SampleRate);
