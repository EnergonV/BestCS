...
namespace BestCS.Controls.Vision
{
    using BestCS.Math;
    using BestCS.Vision.Detection;
    using BestCS.Vision.Detection.Cascades;
    using BestCS.Vision.Tracking;
    using BestCS;
        ...
        IBindableComponent, INotifyPropertyChanged, IVideoSource
    {
        ...
        */
        ...
        private void computeCurrentPosition()
        {
            ...
            currentX = (float)BestCS.Math.Tools.Scale(xaxisRange, unit, obj.Center.X);
            currentY = (float)BestCS.Math.Tools.Scale(yaxisRange, unit, obj.Center.Y);
            currentAngle = (float)BestCS.Math.Tools.Scale(angleRange, circle, obj.Angle);
            currentScale = (float)BestCS.Math.Tools.Scale(scaleRange, unit, Math.Sqrt(obj.Area));
        }
