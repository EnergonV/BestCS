...
namespace BestCS.Neuro.Learning
{
    ...
    /// </remarks>
    /// 
    public class LevenbergMarquardtLearning : BestCS.Neuro.Learning.ISupervisedLearning
    {
        ...
        private static double[][,] createCoefficients(int points)
        {
            ...
            for (int i = 0; i < coefficients.Length; i++)
            {
                ...
                for (int j = 0; j < points; j++)
                {
                    ...
                    {
                        delts[j, k] = hterm / BestCS.Math.Special.Factorial(k);
                        hterm *= delt;
                    }
                }

                coefficients[i] = Matrix.Inverse(delts);
                double fac = BestCS.Math.Special.Factorial(points);
