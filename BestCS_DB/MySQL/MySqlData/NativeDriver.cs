...
using System.Text;
using MySqlProvider.Authentication;
...
namespace MySqlProvider
{
  ...
  internal class NativeDriver : IDriver
  {
    ...
    public void Open()
    {
      ...
      if (baseStream == null)
        throw new MySqlException("Не удаётся подключиться к хосту",
    ...
    private X509CertificateCollection GetClientCertificates()
    {
      ...
      if (Settings.CertificateFile != null)
      {
        if (!Version.isAtLeast(5, 1, 0))
          throw new MySqlException(SqlConsoleProvider.Properties.Resources.FileBasedCertificateNotSupported);
