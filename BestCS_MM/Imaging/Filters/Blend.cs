...
namespace BestCS.Imaging.Filters
{
    ...
    using Matrix = BestCS.Math.Matrix;
    ...
    /// 
    public class Blend : BestCS.Imaging.Filters.BaseTransformationFilter
    {
        ...
        protected override Size CalculateNewImageSize(UnmanagedImage sourceData)
        {
            ...
            float maxY = BestCS.Math.Matrix.Max(py);
            float minY = BestCS.Math.Matrix.Min(py);
                        ...
                        if ((ox >= 0) && (oy >= 0) && (ox < width) && (oy < height))
                        {
                            ...
                            else if (dst[3] > 0)
                            {
                                ...
                                if (Gradient)
                                {
                                    ...
                                    f1 = BestCS.Math.Tools.Scale(0, dmax, 0, 1, d1 - d2);
