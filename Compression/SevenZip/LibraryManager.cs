...
using System.Text;
#if MONO
using BestCS.Compression.SevenZip.Mono.COM;
#endif

namespace BestCS.Compression.SevenZip
{
#if UNMANAGED
    /// <summary>
...
#else
						result = BestCS.Compression.SevenZip.Mono.Factory.CreateInterface<IInArchive>(user, classID, interfaceId);
#endif
                    }
                    catch (Exception)
                    {
 ...
 typeof(IOutArchive).GUID;
                    ...
                    try
                    {
                    	...
                    	NativeMethods.CreateCOMObject(ref classID, ref interfaceId, out result);
#else
						result = BestCS.Compression.SevenZip.Mono.Factory.CreateInterface<IOutArchive>(classID, interfaceId, user);
#endif
                    }
                    catch (Exception)
                    {
