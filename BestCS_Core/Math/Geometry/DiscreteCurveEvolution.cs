...
namespace BestCS.Math.Geometry
{
    using System;
    ...
    public class DiscreteCurveEvolution : IShapeOptimizer
    {
        ...
        public List<IntPoint> OptimizeShape(List<IntPoint> shape)
        {
            ...
            var complex = new List<System.Numerics.Complex>();
            for (int i = 0; i < shape.Count; i++)
                complex.Add(new System.Numerics.Complex(shape[i].X, shape[i].Y));
        ...
        private static double[] winkel(List<System.Numerics.Complex> z)
        {
            int n = z.Count;
            ...
            for (int j = 1; j < n - 1; j++)
            {
                System.Numerics.Complex c = z[j - 1] - z[j + 1];
