...
namespace BestCS.Statistics.Models.Markov.Topology
{
    using System;
    using BestCS.Math;
    ...
    /// <seealso cref="BestCS.Statistics.Models.Markov.Topology.ITopology"/>
    /// <seealso cref="BestCS.Statistics.Models.Markov.Topology.Ergodic"/>
    /// <seealso cref="BestCS.Statistics.Models.Markov.Topology.Custom"/>
    ///
    /// 
    /// <example>
    ///  <para>
        ...
        public int Create(bool logarithm, out double[,] transitionMatrix, out double[] initialState)
        {
            ...
            if (random)
            {
                ...
                for (int i = 0; i < states; i++)
                    sum += pi[i] = BestCS.Math.Tools.Random.NextDouble();
                ...
                for (int i = 0; i < states; i++)
                {
                    sum = 0.0;
                    for (int j = i; j < m; j++)
                        sum += A[i, j] = BestCS.Math.Tools.Random.NextDouble();
