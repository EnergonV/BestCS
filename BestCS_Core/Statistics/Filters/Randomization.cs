...
namespace BestCS.Statistics.Filters
{
    using System;
    using System.Data;
    using BestCS.Math;
    ...
    public class Randomization : IFilter
    {
        ...
        public DataTable Apply(DataTable data)
        {
            ...
            int r = BestCS.Math.Tools.Random.Next();
            if (Seed.HasValue) 
                BestCS.Math.Tools.SetupGenerator(Seed.Value);

            BestCS.Statistics.Tools.Shuffle(indices);

            if (Seed.HasValue)
                BestCS.Math.Tools.SetupGenerator(r);
