...
namespace BestCS.Statistics.Testing
{
    ...
    /// 
    /// <seealso cref="FTest"/>
    /// <seealso cref="BestCS.Statistics.Distributions.Univariate.FDistribution"/>
    /// 
    [Serializable]
    public class LeveneTest : FTest
    {
        ...
        private void compute(double[][] samples, LeveneTestMethod method, double percent)
        {
            ...
            switch (method)
            {
                case LeveneTestMethod.Mean:
                    for (int i = 0; i < means.Length; i++)
                        means[i] = BestCS.Statistics.Tools.Mean(samples[i]);
                    break;

                case LeveneTestMethod.Median:
                    for (int i = 0; i < means.Length; i++)
                        means[i] = BestCS.Statistics.Tools.Median(samples[i]);
                    break;

                case LeveneTestMethod.TruncatedMean:
                    for (int i = 0; i < means.Length; i++)
                        means[i] = BestCS.Statistics.Tools.TruncatedMean(samples[i], percent);
                    break;
            }
            ...
            for (int i = 0; i < newMeans.Length; i++)
                newMeans[i] = BestCS.Statistics.Tools.Mean(z[i]);
