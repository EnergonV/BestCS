...
namespace BestCS.Statistics.Distributions.Multivariate
{
    using System;
    using BestCS.Math;
        ...
        ISampleableDistribution<double[]>
    {
        ...
        public void Fit(double[][] observations, double[] weights, NormalOptions options)
        {
            ...
            if (weights != null)
            {
                ...
                means = BestCS.Statistics.Tools.WeightedMean(observations, weights);

                // Compute weighted covariance matrix
                if (options != null && options.Diagonal)
                    cov = Matrix.Diagonal(BestCS.Statistics.Tools.WeightedVariance(observations, weights, means));
                else cov = BestCS.Statistics.Tools.WeightedCovariance(observations, weights, means);
            }
            else
            {
                // Compute mean vector
                means = BestCS.Statistics.Tools.Mean(observations);

                // Compute covariance matrix
                if (options != null && options.Diagonal)
                    cov = Matrix.Diagonal(BestCS.Statistics.Tools.Variance(observations, means));
                cov = BestCS.Statistics.Tools.Covariance(observations, means);
            }
        ...
        /// actually represents a <see cref="BestCS.Statistics.Distributions.Univariate.NormalDistribution"/>.</returns>
        /// 
