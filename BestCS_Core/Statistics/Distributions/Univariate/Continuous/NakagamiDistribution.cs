...
namespace BestCS.Statistics.Distributions.Univariate
{
        ...
        ISampleableDistribution<double>, IFormattable
    {
        ...
        public override void Fit(double[] observations, double[] weights, IFittingOptions options)
        {
            ...
            if (weights == null)
            {
                mean = BestCS.Statistics.Tools.Mean(x2);
                var = BestCS.Statistics.Tools.Variance(x2, mean);
            }
            else
            {
                mean = BestCS.Statistics.Tools.WeightedMean(x2, weights);
                var = BestCS.Statistics.Tools.WeightedVariance(x2, weights, mean);
            }
