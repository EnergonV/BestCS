...
namespace BestCS.Statistics.Distributions.Univariate
{
    using System;
    using BestCS.Math;
    using BestCS.Statistics.Distributions;
    using BestCS.Statistics.Distributions.Fitting;
    using BestCS.Statistics.Distributions.Multivariate;
    using BestCS;
    ...
    /// </example>
    /// 
    /// <seealso cref="BestCS.Statistics.Distributions.Univariate.SkewNormalDistribution"/>
    /// <seealso cref="BestCS.Statistics.Distributions.Multivariate.MultivariateNormalDistribution"/>
    /// 
    /// <seealso cref="BestCS.Statistics.Testing.ZTest"/>
    /// <seealso cref="BestCS.Statistics.Testing.TTest"/>
    /// 
    [Serializable]
        ...
        ISampleableDistribution<double>, IFormattable
    {
        ...
        /// 
        /// <value>
        ///   A <see cref="BestCS.DoubleRange" /> containing
        ...
        ///  as <see cref="BestCS.Math.Special.Erfc">erfc</see>(-z/sqrt(2)) / 2.</para>  
        ///  
        /// <para>    
        ///   References:
        ///   <list type="bullet">
        ...
        public void Fit(double[] observations, double[] weights, NormalOptions options)
        {
            ...
            if (weights != null)
            {
                ...
                mu = BestCS.Statistics.Tools.WeightedMean(observations, weights);

                // Compute weighted variance
                var = BestCS.Statistics.Tools.WeightedVariance(observations, weights, mu);
            }
            else
            {
                // Compute weighted mean
                mu = BestCS.Statistics.Tools.Mean(observations);

                // Compute weighted variance
                var = BestCS.Statistics.Tools.Variance(observations, mu);
            }
        ...
        public override double[] Generate(int samples)
        {
            ...
            var g = new BestCS.Math.Random.GaussianGenerator(
                (float)mean, (float)stdDev, BestCS.Math.Tools.Random.Next());
