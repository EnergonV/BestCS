...
namespace BestCS.Statistics.Distributions.Univariate
{
    ...
    using BestCS;
    using BestCS.Math.Optimization;
        ...
        IFittableDistribution<double, BetaOptions>, ISampleableDistribution<double>
    {
        ...
        private void initialize(double alpha, double beta)
        {
            ...
            this.constant = 1.0 / BestCS.Math.Beta.Function(alpha, beta);
            this.entropy = null;
        }
        ...
        public override double Entropy
        {
            get
            {
                if (entropy == null)
                {
                    double lnBab = Math.Log(BestCS.Math.Beta.Function(alpha, beta));
        ...
        ///   
        /// <para>
        ///   The Beta's CDF is computed using the <see cref="BestCS.Math.Beta.Incomplete">Incomplete 
        ///   (regularized) Beta function I_x(a,b)</see> as CDF(x) = I_x(a,b)</para>
        /// </remarks>
        /// 
        public override double DistributionFunction(double x)
        {
            return BestCS.Math.Beta.Incomplete(alpha, beta, x);
        }
        ...
        {
            return BestCS.Math.Beta.IncompleteInverse(alpha, beta, p);
        }
        ...
        ///   where constant c is c = 1.0 / <see cref="BestCS.Math.Beta.Function">Beta.Function(a, b)</see></para>
        /// </remarks>
        /// 
        ...
        public static double LogLikelihood(double sum1, double sum2, double n, double alpha, double beta)
        {
            ...
            double t3 = BestCS.Math.Beta.Log(alpha, beta);
        ...
        public static double[] Random(double alpha, double beta, int samples)
        {
            ...
            if (alpha < 1)
            {
                ...
                {
                    double U = BestCS.Math.Tools.Random.Next();
