...
namespace BestCS.MachineLearning
{
    using System;
    using BestCS.Math;
    ...
    public class KMeans : IClusteringAlgorithm<double[]>
    {
        ...
        /// 
        public KMeans(int k)
            : this(k, BestCS.Math.Distance.SquareEuclidean) { }
        ...
        public void Randomize(double[][] points, bool useSeeding)
        {
            ...
            if (UseCentroidSeeding)
            {
                ...
                centroids[0] = (double[])points[BestCS.Math.Tools.Random.Next(0, points.Length)].Clone();
            ...
            else
            {
                ...
                int[] idx = BestCS.Statistics.Tools.RandomSample(points.Length, K);
            ...
            if (ComputeInformation)
            {
                ...
                for (int i = 0; i < centroids.Length; i++)
                {
                    ...
                    if (sub.Length > 0)
                    {
                        ...
                        clusters.Covariances[i] = BestCS.Statistics.Tools.Covariance(sub, centroids[i]);
                    }
                    else
                    {
