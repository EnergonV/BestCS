...
namespace BestCS.MachineLearning.Structures
{
    using System;
    ...
    /// tree.Distance = BestCS.Math.Distance.Manhattan;
    /// 
    ...
    /// 
    /// ' We can also use alternate distance functions
    /// tree.Distance = Function(a, b) BestCS.Math.Distance.Manhattan(a, b)
    /// 
    /// ' And also query for a fixed number of neighbor points
    /// ' (answer should be the points at (5,4), (7,2), (2,3))
    /// '
    ...
    public class KDTree<T> : IEnumerable<KDTreeNode<T>>
    {
        ...
        private Func<double[], double[], double> distance = BestCS.Math.Distance.Euclidean;
        ...
        public Func<double[], double[], double> Distance
        {
            ...
            set
            {
                if (!BestCS.Math.Distance.IsMetric(value))
