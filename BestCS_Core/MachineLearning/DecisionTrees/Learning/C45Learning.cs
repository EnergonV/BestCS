// BestCS Machine Learning Library
...
namespace BestCS.MachineLearning.DecisionTrees.Learning
{
    using System;
    ...
    public class C45Learning
    {
        ...
        private void split(DecisionNode root, double[][] input, int[] output, int height)
        {
            ...
            double entropy = BestCS.Statistics.Tools.Entropy(output, outputClasses);
            ...
            {
                root.Output = BestCS.Statistics.Tools.Mode(output);
                return;
            }
            ...
            else
            {
                ...
                outputSubset = output.Submatrix(maxGainPartition[0]);
                root.Output = BestCS.Statistics.Tools.Mode(outputSubset);
            }
            ...
            int attributeIndex, out int[][] partitions)
        {
            ...
            for (int i = 0; i < partitions.Length; i++)
            {
                ...
                double e = BestCS.Statistics.Tools.Entropy(outputSubset, outputClasses);
            ...
            int attributeIndex, out int[][] partitions, out double threshold)
        {
            ...
            for (int i = 0; i < t.Length; i += splitStep)
            {
                ...
                double splitGain =
                    -p1 * BestCS.Statistics.Tools.Entropy(output1, outputClasses) +
                    -p2 * BestCS.Statistics.Tools.Entropy(output2, outputClasses);
